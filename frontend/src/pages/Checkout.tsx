...
const [showUpiModal, setShowUpiModal] = useState(false);
  ...
  const createOrder = async (paymentMethod: "razorpay" | "stripe" | "cod" | "upi") => {
  ...
  const payWithUPI = async () => {
    try {
      ...
      console.log('UPI QR response', data);
      const { qrUrl } = data;
      setUpiQrUrl(qrUrl);
      console.log('QR URL set to', qrUrl);
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        {loadingAddress ? (
          ...
          <p className="text-sm text-gray-500">
            No address found. Please click 'Add New Address' to create one.
          </p>
        ) : (
          addresses.map((add) => (
            <label
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        {showUpiModal && upiQrUrl && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" onClick={() => setShowUpiModal(false)}>
            <div className="bg-white p-4 rounded-lg" onClick={e => e.stopPropagation()}>
              ...
              <img src={upiQrUrl} alt="UPI QR" className="w-64 h-64" />
              <button className="mt-4 px-4 py-2 bg-red-500 text-white rounded" onClick={() => setShowUpiModal(false)}>Close</button>
            </div>
          </div>
        )}
    </div>
